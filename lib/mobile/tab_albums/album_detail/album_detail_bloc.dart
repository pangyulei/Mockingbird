...
class AlbumDetailBloc extends AlbumDetailBlocType {
  ...
  AlbumDetailBloc(this._albumId) : super(const AlbumDetailInitState()) {
    ...
    on<AlbumDetailAppResumeEvent>(_onAppResume);
    _subscriptionList.addAll([EventHub.on<HubAppResumeEvent>((event) => add(const AlbumDetailAppResumeEvent()),)]);
  }
  ...
  void _onAppResume(AlbumDetailAppResumeEvent event, Emitter<AlbumDetailState> emit) async {
    EasyLoading.show(maskType: .clear);
    emit(await _reload());
    EasyLoading.dismiss();
  }
